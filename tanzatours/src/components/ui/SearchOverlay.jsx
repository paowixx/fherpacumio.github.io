...
const SearchOverlay = ({ isOpen, onClose }) => {
  ...
  const mockSuggestions = [
    { id: 1, title: 'Corregidor Island', type: 'Historical Site', location: 'Cavite' },
    { id: 2, title: 'Tagaytay Ridge', type: 'Scenic Viewpoint', location: 'Cavite' },
    { id: 3, title: 'Aguinaldo Shrine', type: 'Museum', location: 'Kawit, Cavite' },
    { id: 4, title: 'Ternate Beach', type: 'Beach Resort', location: 'Ternate, Cavite' },
    { id: 5, title: 'People\'s Park in the Sky', type: 'Park', location: 'Tagaytay' },
  ];
  ...
  const handleSearch = (query) => {
    if (query.trim()) {
      ...
      navigate(`/search-and-filter-results?q=${encodeURIComponent(query)}`);
      onClose();
    }
  };
        ...
        <form onSubmit={handleSubmit} className="flex-1">
          <div className="relative">
            ...
            <Input
              ref={searchInputRef}
              type="search"
              placeholder="Search destinations, activities, events..."
              value={searchQuery}
      ...
      <div className="flex-1 overflow-y-auto p-4">
              ...
              {suggestions.map((suggestion) => (
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="font-medium text-card-foreground truncate">{suggestion.title}</div>
        ...
        {!isLoading && searchQuery && suggestions.length === 0 && (
          <div className="text-center py-8">
            ...
            <div className="text-muted-foreground">Try searching for destinations, activities, or events</div>
          </div>
        )}
        ...
        {!searchQuery && recentSearches.length === 0 && (
          <div className="space-y-3">
            ...
            <div className="space-y-2">
              {['Historical Sites', 'Beach Resorts', 'Mountain Views', 'Local Cuisine', 'Adventure Activities'].map((query, index) => (
                <button
                  key={index}
                  onClick={() => handleRecentSearchClick(query)}
                  className="flex items-center w-full p-3 text-left bg-muted rounded-lg hover:bg-accent/10 transition-colors"
        ...
        <div className="mt-8 space-y-3">
          ...
          <div className="flex flex-wrap gap-2">
            {['Nearby', 'Open Now', 'Free Entry', 'Family Friendly', 'Pet Friendly'].map((filter) => (
              <Button
                key={filter}
                variant="outline"
                size="sm"
